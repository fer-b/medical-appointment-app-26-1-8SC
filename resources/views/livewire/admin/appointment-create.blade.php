        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <form wire:submit.prevent="searchAvailability" class="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div class="flex-1">
                        <label class="block mb-2 text-sm font-medium text-gray-900">Especialidad (opcional)</label>
                        <input wire:model="searchSpecialty" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Ej: Cardiología">
                    </div>
                    <div>
            ...
            <div class="space-y-4">
                @forelse($availableDoctors as $doctor)
                    <div class="bg-white rounded-lg shadow p-6 border {{ $selectedDoctor && $selectedDoctor->id === $doctor->id ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-200' }}">
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                <h3 class="font-bold text-gray-900">{{ $doctor->user->name ?? 'Doctor sin nombre' }}</h3>
                                <p class="text-sm text-blue-600">{{ $doctor->specialty ?? 'General' }}</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                @foreach(['08:00:00', '09:00:00', '10:30:00', '14:00:00'] as $time)
                                    <button wire:click="selectTimeSlot({{ $doctor->id }}, '{{ $time }}')" type="button" 
                                            class="{{ $selectedDoctor && $selectedDoctor->id === $doctor->id && $selectedTime === $time ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200' }} focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-1.5 text-center transition-colors">
                ...
                @empty
                    <div class="text-center py-8 text-gray-500 bg-white rounded-lg shadow">
                        No se encontraron doctores disponibles con esos criterios.
                    </div>
                @endforelse
            </div>
                ...
                <form wire:submit.prevent="confirmAppointment">
                    ...
                    <div class="space-y-3 mb-6">
                        <div class="flex justify-between border-b border-gray-100 pb-2">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $selectedDoctor ? $selectedDoctor->user->name : 'No seleccionado' }}</span>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-2">
                    ...
                    <div class="mb-4">
                        ...
                        <select wire:model="patient_id" class="block w-full p-2.5 text-sm rounded-lg @error('patient_id') bg-red-50 border border-red-500 text-red-900 focus:ring-red-500 focus:border-red-500 @else bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 @enderror">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea wire:model="reason" rows="3" class="block p-2.5 w-full text-sm rounded-lg @error('reason') bg-red-50 border border-red-500 text-red-900 focus:ring-red-500 focus:border-red-500 @else bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 @enderror" placeholder="Ej: Chequeo de medicamentos"></textarea>
                        @error('reason') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <!-- Validation Errors for Selection -->
                    @error('selectedDoctor') <span class="text-xs text-red-600 mb-2 block text-center">{{ $message }}</span> @enderror
